...
int main(void)
{
	...
	// Initialize the AREF - block needed by SAR ADC
    ...
    for (;;){
		...
		if(Cy_SAR_IsEndConversion(SAR, CY_SAR_WAIT_FOR_RESULT) == CY_SAR_SUCCESS){
			ADCresult = Cy_SAR_GetResult32(SAR, 0);
			microVolts = Cy_SAR_CountsTo_uVolts(SAR, 0, ADCresult);
		}
		char stringBuffer[20];
		sprintf(stringBuffer, "%ld\n", microVolts);
		Cy_SCB_UART_PutString(SCB5, stringBuffer);
		Cy_SysLib_Delay(100);
    }
}
