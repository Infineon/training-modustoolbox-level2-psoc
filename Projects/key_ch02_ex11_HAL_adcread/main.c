...
int main(void)
{
	...
	/* Initialize ADC. The ADC block which can connect to the pin that is connected to the potentiometer is selected */
	result = cyhal_adc_init(&adc_obj, CYBSP_POT, NULL);
	...
	result = cyhal_adc_channel_init_diff(&adc_chan_0_obj, &adc_obj, CYBSP_POT, CYHAL_ADC_VNEG, &channel_config);
