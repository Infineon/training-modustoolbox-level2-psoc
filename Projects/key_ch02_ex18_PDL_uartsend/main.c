...
void Interrupt_Handler_Port3(void){
	...
	if(PORT3_INTR_MASK == (intrSrc & PORT3_INTR_MASK)){
		...
		Cy_GPIO_ClearInterrupt(P3_7_PORT, P3_7_NUM);
		// Toggle LED
		Cy_GPIO_Inv(P3_4_PORT, P3_4_NUM);
		pressedFlag = true;
	}
}
...
int main(void)
{
	...
	Cy_GPIO_SetFilter(P3_7_PORT, P3_7_NUM);
	// Falling edge interrupt
	Cy_GPIO_SetInterruptEdge(P3_7_PORT, P3_7_NUM, CY_GPIO_INTR_FALLING);
    ...
    for (;;){
    	...
    	if(pressedFlag){
    		...
    		Cy_SCB_UART_Put(SCB3, dataBuffer);
    		pressedFlag = false;
    	}
    }
}
