 ...
 */
void I2C_Isr(void){
    Cy_SCB_I2C_MasterInterrupt(SCB3, &I2C_context);
}
...
static int8_t i2c_write_bytes(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint16_t len){
    ...
    // Write
    cy_en_scb_i2c_status_t result = Cy_SCB_I2C_MasterWrite(SCB3, &xferConfig, &I2C_context);
...
static int8_t i2c_read_bytes(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint16_t len){
    ...
    if (CY_SCB_I2C_SUCCESS == result){
		...
		// Read
		result = Cy_SCB_I2C_MasterRead(SCB3, &xferConfig, &I2C_context);
...
int main(void)
{
	...
	Cy_SCB_I2C_Init(SCB3, &scb_3_config, &I2C_context);
    ...
    if(status != BMI160_OK){
    	Cy_SCB_UART_PutString(SCB5, "Motion Sensor Initialization Failed!\n");
    	CY_ASSERT(0);
    }
	...
	if(status != BMI160_OK){
		Cy_SCB_UART_PutString(SCB5, "Motion Sensor Configuration Failed!\n");
		CY_ASSERT(0);
	}
    ...
    for (;;){
    	...
    	if(status != BMI160_OK){
			Cy_SCB_UART_PutString(SCB5, "Motion Sensor Read Failed!\n");
			CY_ASSERT(0);
		}
    	char printBuffer[50];
    	sprintf(printBuffer, "Accel: X:%6d Y:%6d Z:%6d\r\n", accel.x, accel.y, accel.z);
    	Cy_SCB_UART_PutString(SCB5, printBuffer);
