...
int main(void)
{
	...
	dma_init_status = Cy_DMA_Descriptor_Init(&UART_DMA_Descriptor_1, &UART_DMA_Descriptor_1_config);
	if(dma_init_status != CY_DMA_SUCCESS){
		CY_ASSERT(0);
	}

	/* Set source and destination for descriptor 0 */
	Cy_DMA_Descriptor_SetSrcAddress(&UART_DMA_Descriptor_0, (void *) buffer0);
	...
	Cy_DMA_Descriptor_SetSrcAddress(&UART_DMA_Descriptor_1, (void *) buffer1);
	Cy_DMA_Descriptor_SetDstAddress(&UART_DMA_Descriptor_1, (void *) &(DEBUG_UART_HW->TX_FIFO_WR));
